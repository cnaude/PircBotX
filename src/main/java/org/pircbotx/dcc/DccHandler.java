...
public class DccHandler implements Closeable {
    ...
    public SendChat sendChat(User receiver, boolean passive) throws IOException, InterruptedException {
        ...
        if (passive) {
            ...
            Socket userSocket;
            try ( //Get the user to connect to us
    ...
    public SendFileTransfer sendFile(File file, User receiver, boolean passive) throws IOException, DccException, InterruptedException {
        ...
        if (passive) {
            ...
            PendingSendFileTransfer pendingSendFileTransfer;
            Socket userSocket;
            try ( //Try to get the user to connect to us
                    ServerSocket serverSocket = createServerSocket(receiver)) {
                pendingSendFileTransfer = new PendingSendFileTransfer(receiver, safeFilename, serverSocket.getLocalPort());
                synchronized (pendingSendTransfers) {
    ...
    public void close() {
        ...
        shuttingDown = true;
        //log.debug("Terminating all transfers waiting to be accepted");
